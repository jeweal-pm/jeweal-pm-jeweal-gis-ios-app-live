...
import UIKit
import Alamofire
class DiamondReservedList : UITableViewCell {
...
class DiamondReservedItems: UIViewController , UITableViewDelegate , UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        mHeadingLABEL.text = "Reserved Items".localizedString
        ...
        if sender.text == "" {
            ...
            for i in mDefaultData {
                ...
                if let mData = i as? NSDictionary {
                    ...
                    let mValue = "\(mData.value(forKey: "customer") ?? "")"
                    let mStockId = "\(mData.value(forKey: "StockID") ?? "")"
                    
                    let serchkey = sender.text ?? ""
                    if mValue.lowercased().contains(serchkey.lowercased()) || mStockId.contains(serchkey)  {
                ...
                if "\(response.value(forKey: "code") ?? "")" == "200" {
                    ...
                    mGetData(url: mDiamondReservedListAPI ,headers:sGisHeaders, params: ["":""]) { response , status in
                        ...
                        if status {
                            ...
                            } else if "\(response.value(forKey: "code") ?? "")" == "400" {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "DiamondReservedList") as? DiamondReservedList else { return UITableViewCell() }
        ...
        if let mData = mData[indexPath.row] as? NSDictionary {
            ...
            if mSelectedItems.isEmpty {
                ...
                mHeight.constant = 50
                mRemoveButton.isHidden = false
            }
            
            if let id = mData.value(forKey: "id") as? String,
               mSelectedItems.contains(id) {
