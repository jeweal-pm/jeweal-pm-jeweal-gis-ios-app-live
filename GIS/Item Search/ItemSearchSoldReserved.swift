    ...
    @IBOutlet weak var mCertificateLABEL: UILabel!
        ...
        override func viewDidLoad() {
            ...
            if mType == "reserve" {
                ...
                print("mReserveSoldAPI mParams = \(mParams)")
                mGetData(url: mReserveSoldAPI ,headers: sGisHeaders,  params: mParams) { response , status in
                    ...
                    if status {
                        ...
                        print("viewDidLoad response = \(response)")
                    if "\(response.value(forKey: "code") ?? "")" == "200" {
                        ...
                        if let mDataItems = response.value(forKey: "data") as? NSArray {
                            ...
                            if let mData = mDataItems[0] as? NSDictionary {
                                print("Customer =", mData.value(forKey: "customer_name") ?? "nil")
                                print("stone_name =", mData["stone_name"] ?? "nil")
...
//                                self.mCollectionName.text = "\(mData.value(forKey: "Collection_name") ?? "--")"
                                self.mCollectionName.text = "\(mData.value(forKey: "Collection_name") ?? mData.value(forKey: "Collection") ?? "--")"
    ...
    @IBAction func mSearchFilters(_ sender: UITextField) {
        ...
        if sender.text == "" {
            ...
            print("tableView cellForRowAt mData = \(mData)")
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if mType == "reserve" {
            ...
            if let mData = mData[indexPath.row] as? NSDictionary {
                ...
                cell.mStockId.font = UIFont.boldSystemFont(ofSize: 14)
